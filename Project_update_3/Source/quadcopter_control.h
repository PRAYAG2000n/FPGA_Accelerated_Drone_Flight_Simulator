...
// Define data types for fixed-point arithmetic - CHANGED float_t to fp32_t
typedef ap_fixed<32,16> fp32_t;  // Changed from float_t to fp32_t
...
struct StateVector {
    fp32_t roll, pitch, yaw;  // Changed from float_t to fp32_t
...
struct SystemParams {
    fp32_t mass;  // Changed from float_t to fp32_t
...
// Function prototypes
void complementary_filter(IMUData imu, StateVector& state, fp32_t alpha);  // Changed parameter type

...
void altitude_controller(StateVector current_state, ControlCommands commands,
    ...
    // HLS-compatible absolute value check for deadband
    if (alt_error < control_signal_t(0)) {
        if (-alt_error < alt_deadband) {
            alt_error = control_signal_t(0);
        }
    } else {
        if (alt_error < alt_deadband) {
            alt_error = control_signal_t(0);
    ...
    throttle_output = throttle_output + hover_throttle;

    // Throttle limiting
    if (throttle_output < control_signal_t(15)) {
        throttle_output = control_signal_t(15);
    }
    if (throttle_output > control_signal_t(85)) {
        throttle_output = control_signal_t(85);
    }
